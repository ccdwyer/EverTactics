  ...
  const material = new THREE.ShaderMaterial({
    ...
    fragmentShader: /* glsl */ `
      ...
      void main() {
        ...
        //
        // ── ROUND 8 ────────────────────────────────────────────────────────────
        // Shot again with the fragment forced to a signal colour and amplified
        // 40x, the mark that survives to the framebuffer is a thin crescent a
        // few pixels deep in front of each unit's boots. Everything else is
        // either behind the billboard or eaten by the two windows below. The
        // lobes are widened here to fill the whole strip of the occupied tile
        // that the lens can actually see — at 45 degrees yaw the tile's diagonal
        ...
        occ *= (1.0 - smoothstep(0.86, 1.0, max(e.x, e.y)));
    ...
    depthTest: true,
    polygonOffset: true,
    // Round 7 took this from −2/−2 down to −1/−1 because the 1.6-tile quad was
    // punching slivers through the wall a unit stands against. Round 8 puts it
    // back up to −2.5/−6, which is the only lever that buys depth clearance
    // without moving the quad on screen — and the bleed it caused is now handled
    // properly instead of by starving the offset: the tile window below closes
    // at 0.99 of the tile half-extent rather than 1.0, so the decal is already
    // at zero before it reaches the geometry it could bleed onto.
 ...
 *
 * ─────────────────────────────────────────────────────────────────────────────
 * ROUND 8 — the lift was under the receiver's own bias, so none of this shipped
 * ─────────────────────────────────────────────────────────────────────────────
 * Grounding scored 2.7/10 again, unchanged for four rounds, and this is why.
 * Measured three ways on 'battle-open':
 *
 *   a. Rendering the caster's *own geometry* in a signal colour: the discs are
 *      exactly where they should be — one under every unit, roughly a tile
 *      across, sitting in fully visible screen space below and right of each
 *      figure. So placement, size and visibility were never the problem.
 *   b. Differencing the frame with 'GROUND_CASTER_RADIUS' at its shipped 0.36
 *      against the same frame with the caster collapsed to nothing: the delta
 *      is under the noise floor everywhere. The discs are in the map and the map
 *      is doing nothing with them.
 *   c. Re-shooting at radius 1.2: shadows appear, but only on terrain a *step
 *      below* the unit — never on the tile the unit is standing on.
 *
 * (c) is the tell. 'fitTo()' sets 'key.shadow.normalBias = texelWorld * 1.6 *
 * shadowNormalBiasScale', which on this map is 0.03–0.05 **world units**, and a
 * normal bias offsets the receiver's lookup along its own normal — straight up,
 * for a floor. The disc was pinned 0.055 above that floor. The receiver was
 * therefore sampling the shadow map from a point at or above the caster, found
 * nothing in front of it, and returned "lit" for every fragment of the tile the
 * unit occupies. Only receivers a whole 'HEIGHT_UNIT' lower had enough clearance
 * to register — exactly what (c) shows.
 *
 * The lift is now 0.34, comfortably clear of any plausible normal bias. That
 * costs a displacement of 'lift * cot(elevation)' ≈ 0.25 tiles down the key's
 * ground heading, which would read as peter-panning if it were left uncorrected;
 * 'update' subtracts it out of the ellipse's lean so the near edge of the mark
 * still sits under the boots. See {@link UnitSprite.shadowCot}.
 *
 ...
 * units, so the shadow is welded to the silhouette instead of starting clear of
 * it. Roughly half a boot. See the lean solve in {@link UnitSprite.update}.
 */
const CASTER_BITE = 0.14;
...
export class UnitSprite {
  ...
  /**
   * cot(key elevation) — how far along the ground one world unit of height
   * throws. Kept raw and unclamped-by-taste (unlike 'shadowStretch', which is
   * squeezed into an authored band) because it is used to *undo* a geometric
   * displacement rather than to author a look: the caster disc floats
   * {@link GROUND_CASTER_LIFT} above the surface, so the mark it writes into the
   * shadow map lands 'lift * shadowCot' further down the light than the unit's
   * feet. 'update' subtracts exactly that from the ellipse's lean, which is what
   * keeps a mark whose lift is 6x what it used to be from reading as a shadow
   * that has come unstuck from the boots.
   */
  private shadowCot = 0.73;
  ...
  setKeyLight(direction: THREE.Vector3, rimColor?: THREE.ColorRepresentation): void {
    ...
    if (groundLength > 1e-5) {
      ...
      this.shadowCot = Math.min(2.2, Math.max(0.2, cot));
    } else {
  ...
  update(dt: number, view: SpriteViewContext): void {
    ...
    if (this.groundCaster) {
      ...
      // Elongate along the key's ground heading, so the mark leans where every
      // terrace and parapet shadow in the frame leans. A perfectly round blob
      // under a 54° key is what reads as a decal; the asymmetry is what says
      // "cast by this light".
      //
      // ── ROUND 8: the lean is now *solved for*, not authored ────────────────
      // The disc floats GROUND_CASTER_LIFT above the surface (see there for why
      // it has to), and a floating caster writes its mark 'lift * cot(elevation)'
      // further down the light than the thing it belongs to. Left uncorrected
      // that is textbook peter-panning — the exact failure the brief calls out
      // by name — so the geometry is inverted instead of guessed at:
      //
      //   nearEdge = feet + (lean + liftShift - halfLength) * dir
      //
      // and we want 'nearEdge' a hair *behind* the boots (CASTER_BITE) so the
      ...
      const liftShift = GROUND_CASTER_LIFT * this.shadowCot;
      const lean = Math.max(0, halfLength - CASTER_BITE - liftShift);
      ...
      this.groundCaster.scale.set(radius, 1, halfLength);
